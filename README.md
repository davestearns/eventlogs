...
Aggregates can be cached aggressively, or even stored for listing/searching purposes, because the events they were calculated from are immutable. As new events related to the aggregate are appended to the log, these cached/saved aggregates can be quickly updated with just the events that occurred _after_ the last time they were calculated.
...
If you need to support listing, searching, or analyzing your aggregates (e.g., show me all payments over the last 30 days that have been fully refunded), it's best to save them to a separate highly-indexed database that gets updated as new events are recorded. These aggregates will be eventually-consistent with the events, but listing/querying APIs are often that way in large distributed systems. This also keeps the writes to the transaction-processing database very fast since those are just inserts to a minimally-indexed table. This division of labor is typically called ["Command and Query Responsibility Separation"](https://learn.microsoft.com/en-us/azure/architecture/patterns/cqrs) or CQRS for short.
